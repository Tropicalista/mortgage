...
const calculator = fields => {
  const result = (0,financial__WEBPACK_IMPORTED_MODULE_1__.pmt)(fields.rate / 100 / fields.frequency, fields.frequency * fields.term, fields.amount);
  return result * -1;
};
const loanResponse = () => {
  const data = [];
  ...
  const ipmts = periods.map(per => (0,financial__WEBPACK_IMPORTED_MODULE_1__.ipmt)(context.rate / 100 / context.frequency, per, 1 * periods.length, Number(context.amount)));
...
const ratesResponse = () => {
  ...
  count.forEach(y => {
    ...
    data.push({
      ...
      interests: formatNumber(result * 12 * context.term - context.amount),
...
const {
  state
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)('mortgage', {
  callbacks: {
    generateTable: () => {
      const context = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.getContext)();
      if (context.result) {
